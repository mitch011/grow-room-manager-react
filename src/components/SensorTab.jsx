...
const SensorTab = ({
  ...
  const latestVpd = sensorData.length > 0 ? sensorData[sensorData.length - 1].vpd.toFixed(2) : '--';
  ...
  const chartOptions = {
    ...
    scales: {
      ...
      y2: { type: 'linear', position: 'right', title: { display: true, text: 'CO₂ (ppm)' }, grid: { drawOnChartArea: false }, offset: true }
    },
  };
  ...
  return (
    <div className="sensor-tab">
      ...
      <div className="control-group">
        ...
        <button onClick={onRecordSensorData}>
          Record Data
        </button>
      </div>

      <div className="sensor-display" style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
        <div className="sensor-card"><div className="sensor-label">Current Temp</div><div className="sensor-value">{latestTemp}</div></div>
        <div className="sensor-card"><div className="sensor-label">Current Humidity</div><div className="sensor-value">{latestHumidity}</div></div>
        <div className="sensor-card"><div className="sensor-label">Current CO₂</div><div className="sensor-value">{latestCo2}</div></div>
        <div className="sensor-card"><div className="sensor-label">VPD</div><div className="sensor-value">{latestVpd}</div></div>
      </div>
